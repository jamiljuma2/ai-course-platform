        ...
        <div className="absolute right-4 bottom-8 sm:top-1/2 sm:bottom-auto sm:-translate-y-1/2 max-w-[320px] w-full sm:w-80 mt-0 mx-auto lg:mx-0">
          <div className="card glow-brand animate-float">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <div className="text-sm font-semibold text-white">Module 2 Progress</div>
                <div className="text-xs text-dark-400">Mastering ChatGPT</div>
              </div>
            </div>
            <div className="space-y-2 mb-4">
              {['Prompt Engineering', 'Role-Based Prompting', 'Custom GPTs'].map((lesson, i) => (
                <div key={lesson} className="flex items-center gap-2 text-sm">
                  <div className={`w-4 h-4 rounded-full border flex items-center justify-center text-xs ${
                    i < 2 ? 'bg-brand-500 border-brand-500 text-white' : 'border-dark-500'
                  }`}>
                    {i < 2 && '✓'}
                  </div>
                  <span className={i < 2 ? 'text-white' : 'text-dark-400'}>{lesson}</span>
                </div>
              ))}
            </div>
            <div className="bg-dark-700 rounded-full h-2">
              <div className="bg-brand-500 h-2 rounded-full w-2/3" />
            </div>
            <div className="text-xs text-dark-400 mt-2">67% complete</div>
          </div>
        </div>
      </div>
    </section>
